...
$BindsFile   = 'docker-compose.binds.yml'
$EnvFile     = '.env.prod'
...
function Read-Config {
    ...
    Write-Host "    Where the database, file blobs and TLS certificates live on this host." -ForegroundColor DarkGray
    Write-Host "    Leave the default to use Docker-managed named volumes." -ForegroundColor DarkGray
    $useBinds = $false; $dataRoot = ''
    if (Ask-YesNo "Store data in a specific host directory (bind mount)?" "N") {
        $dataRoot = Ask "Data directory" (Join-Path $HOME 'filesync-data')
        $dataRoot = [System.IO.Path]::GetFullPath((Join-Path (Get-Location) $dataRoot))
        $useBinds = $true
        Ok "File blobs -> $dataRoot\blobs"
        Ok "Database   -> $dataRoot\postgres"
        Ok "TLS certs  -> $dataRoot\caddy"
    } else { Info "Using Docker named volumes." }
    ...
    $port    = Ask "Internal server port" "3001" -Optional
    $storage = Ask "Blob storage path inside the container" "./data/blobs" -Optional
    $nodeEnv = Ask "NODE_ENV" "production" -Optional

    $script:Cfg = @{
        Domain = $domain; ServerUrl = $serverUrl; Cors = $cors; Port = $port
        NodeEnv = $nodeEnv; Storage = $storage; UseBinds = $useBinds; DataRoot = $dataRoot
...
"@
    ...
    if ($c.UseBinds) {
        foreach ($sub in 'postgres','blobs','caddy') { New-Item -ItemType Directory -Force -Path (Join-Path $c.DataRoot $sub) | Out-Null }
        # Docker (Linux engine) needs forward-slash paths.
        $root = ($c.DataRoot -replace '\\','/')
        $binds = @"
# Generated by scripts/setup.ps1 - binds the named volumes to host directories.
volumes:
  postgres_data:
    driver: local
    driver_opts:
  ...
  caddy_data:
    ...
    driver_opts:
      ...
      device: $root/caddy
"@
        Set-Content -Path $BindsFile -Value $binds -Encoding ASCII
        Ok "Wrote $BindsFile (host bind mounts)"
    }
    elseif (Test-Path $BindsFile) { Remove-Item $BindsFile }
}

function Get-ComposeArgs {
    if ($script:Cfg.UseBinds) { return @('--env-file', $EnvFile, '-f', $ComposeFile, '-f', $BindsFile) }
    else { return @('--env-file', $EnvFile, '-f', $ComposeFile) }
}
